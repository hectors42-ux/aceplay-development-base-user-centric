...
BEGIN
  ...
  SELECT reason::text INTO _last_reason
  ...
  -- Partidos recientes con marcador y partner cuando aplica (top 10)
  ...
  FROM (
    SELECT
      ...
      CASE
        ...
        ELSE NULL
      END AS opponent_id,
      -- Marcador del partido (string "6-3, 4-6, 7-5") si está disponible
      CASE
        WHEN rh.source::text = 'ladder_challenge' THEN
      ...
      -- Nombre del partner (dobles) en torneos
      CASE
        WHEN rh.source::text = 'tournament_match' THEN
          (SELECT
             CASE
...
-- Helper: convierte un score jsonb a string tipo "6-3, 4-6, 7-5".
-- Acepta varios formatos: {sets: [{a:6,b:3},...]}, {sets: [[6,3],[4,6]]}, [[6,3],[4,6]].
CREATE OR REPLACE FUNCTION public.format_score_summary(_score jsonb)
RETURNS text
LANGUAGE plpgsql
IMMUTABLE
AS $$
DECLARE
  ...
  _b text;
BEGIN
  IF _score IS NULL THEN RETURN NULL; END IF;

  IF jsonb_typeof(_score) = 'array' THEN
    _sets := _score;
  ELSIF _score ? 'sets' AND jsonb_typeof(_score->'sets') = 'array' THEN
    _sets := _score->'sets';
  ELSE
    RETURN NULL;
  END IF;

  IF jsonb_array_length(_sets) = 0 THEN RETURN NULL; END IF;

  FOR _set IN SELECT * FROM jsonb_array_elements(_sets) LOOP
    IF jsonb_typeof(_set) = 'array' AND jsonb_array_length(_set) >= 2 THEN
      _a := _set->>0;
      _b := _set->>1;
    ELSIF jsonb_typeof(_set) = 'object' THEN
      _a := COALESCE(_set->>'a', _set->>'home', _set->>'p1', _set->>'player_a');
      _b := COALESCE(_set->>'b', _set->>'away', _set->>'p2', _set->>'player_b');
    ELSE
      _a := NULL; _b := NULL;
    END IF;

    IF _a IS NOT NULL AND _b IS NOT NULL THEN
      _parts := array_append(_parts, _a || '-' || _b);
    END IF;
  END LOOP;

  IF array_length(_parts, 1) IS NULL THEN RETURN NULL; END IF;
