CREATE OR REPLACE FUNCTION public.analytics_finance_summary(p_from timestamp with time zone, p_to timestamp with time zone)
 RETURNS jsonb
 LANGUAGE plpgsql
...
CREATE OR REPLACE FUNCTION public.analytics_overview(p_from timestamp with time zone, p_to timestamp with time zone)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  INTO v_total_slot_seconds FROM public.courts c WHERE c.tenant_id = v_tenant AND c.is_active = true;

  SELECT COALESCE(SUM(EXTRACT(EPOCH FROM (b.ends_at - b.starts_at))), 0) INTO v_booked_seconds
  FROM public.bookings b WHERE b.tenant_id = v_tenant AND b.status = 'confirmada' AND b.starts_at >= p_from AND b.starts_at < p_to;

  SELECT COALESCE(SUM(EXTRACT(EPOCH FROM (b.ends_at - b.starts_at))), 0) INTO v_prev_booked_seconds
  FROM public.bookings b WHERE b.tenant_id = v_tenant AND b.status = 'confirmada' AND b.starts_at >= v_prev_from AND b.starts_at < v_prev_to;
  ...
  WHERE p.tenant_id = v_tenant AND NOT EXISTS (SELECT 1 FROM public.bookings b WHERE b.user_id = p.user_id AND b.starts_at >= now() - interval '30 days');
  ...
  WHERE tenant_id = v_tenant AND status IN ('propuesto','aceptado','programado');

  BEGIN
    SELECT
      (SELECT COUNT(*) FROM public.ladder_challenges WHERE tenant_id = v_tenant AND played_at >= now() - interval '7 days' AND status = 'jugado')
  ...
  SELECT COALESCE(SUM(price_clp), 0) INTO v_clases_revenue FROM public.coach_class_bookings
  WHERE tenant_id = v_tenant AND payment_status = 'pagada' AND starts_at >= p_from AND starts_at < p_to;
  ...
  SELECT COALESCE(jsonb_agg(t), '[]'::jsonb) INTO v_top_coaches FROM (
    ...
    GROUP BY cp.id, p.first_name, p.last_name ORDER BY classes DESC LIMIT 5
  ) t;
