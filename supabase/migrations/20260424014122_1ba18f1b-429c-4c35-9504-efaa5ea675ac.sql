...
BEGIN
  ...
  -- Pendientes (solo perfil propio)
  IF v_is_self THEN
    -- Torneos: distinguir 'submit' / 'confirm' / 'wait' según quién propuso el resultado
    ...
    FROM (
      SELECT
        ...
        -- needs_action consistente con ladder: 'submit' | 'confirm' | 'wait'
        CASE
          WHEN EXISTS (
            SELECT 1 FROM public.tournament_match_results tmr
