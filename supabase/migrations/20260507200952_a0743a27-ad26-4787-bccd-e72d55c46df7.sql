CREATE OR REPLACE FUNCTION public.create_booking(_court_id uuid, _starts_at timestamp with time zone, _partner_user_id uuid, _notes text DEFAULT NULL::text, _duration_minutes integer DEFAULT NULL::integer)
 RETURNS bookings
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Pre-check informativo (no atómico): cancha
  ...
  -- Pre-check informativo: compañero
  ...
  -- INSERT atómico: el constraint EXCLUDE bookings_no_overlap garantiza no race conditions
  BEGIN
