-- PRD 12 §1.1 · Binding ronda ↔ sesión

-- 1. Columna nueva
ALTER TABLE public.americano_rounds
  ADD COLUMN IF NOT EXISTS tournament_session_id uuid REFERENCES public.tournament_sessions(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS americano_rounds_session_idx
  ON public.americano_rounds (tournament_session_id);

-- 2. Backfill por ventana horaria (created_at de la ronda dentro de [starts_at, ends_at] de la sesión)
UPDATE public.americano_rounds r
SET tournament_session_id = (
  SELECT s.id
  FROM public.tournament_sessions s
  JOIN public.tournament_categories tc ON tc.id = r.tournament_category_id
  WHERE s.tournament_id = tc.tournament_id
    AND s.starts_at <= r.created_at
    AND s.ends_at   >= r.created_at
  ORDER BY s.starts_at
  LIMIT 1
)
WHERE r.tournament_session_id IS NULL;

-- 3. RPC: persistir _session_id en la inserción de la ronda
...
BEGIN
  ...
  INSERT INTO public.americano_rounds (tenant_id, tournament_category_id, round_number, status, tournament_session_id)
  VALUES (v_cat.tenant_id, _category_id, _round_number, 'pendiente', _session_id)
