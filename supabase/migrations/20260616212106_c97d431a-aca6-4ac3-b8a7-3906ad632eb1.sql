...
BEGIN
  ...
  -- Jugadores: demouser (+ hector si existe) + bots demo
  ...
  -- Cuartos (round = max round del playoff)
  FOR v_match IN
  ...
  IF _state = 'dia3' THEN
    FOR v_match IN
      SELECT id, registration_a_id AS a, registration_b_id AS b, next_match_id, next_match_slot
        FROM public.tournament_matches
       WHERE tournament_category_id = v_cat_id AND phase = 'playoff'
         AND status = 'pendiente' AND round = 2
         AND registration_a_id IS NOT NULL AND registration_b_id IS NOT NULL
    ...
    LOOP
      DECLARE
        v_winner uuid := CASE WHEN v_match.a = v_reg_ids[1] OR v_match.b = v_reg_ids[1]
                              THEN v_reg_ids[1]
                              ELSE CASE WHEN random() < 0.5 THEN v_match.a ELSE v_match.b END END;
        v_score jsonb := jsonb_build_object('sets', jsonb_build_array(jsonb_build_object('a', 6, 'b', (random()*4)::int)));
      BEGIN
        UPDATE public.tournament_matches
           SET winner_registration_id = v_winner, score = v_score,
  ...
  -- Finalizado: jugar todo lo restante
  FOR v_match IN
    SELECT id, registration_a_id AS a, registration_b_id AS b, round, next_match_id, next_match_slot
      FROM public.tournament_matches
     WHERE tournament_category_id = v_cat_id AND phase = 'playoff'
