...
-- Ajuste mínimo: cambiar bracket_position de cuartos/semis/final a 101+ para evitar
-- colisión con la constraint UNIQUE (tournament_id, bracket, round, bracket_position)
-- que comparte espacio con los partidos de grupos.
...
BEGIN
  ...
  -- Playoff: cuartos (round 3, bp 101..104), semis (round 2, bp 101..102), final (round 1, bp 101)
  ...
  FOR v_i IN 1..2 LOOP
    INSERT INTO public.tournament_matches (
      ...
      v_tour_id, v_cat_id, v_tenant,
      2, 100 + v_i, 'playoff', 'pendiente'::match_status
    ) RETURNING id INTO v_id_tmp;
  ...
  INSERT INTO public.tournament_matches (
    ...
    v_tour_id, v_cat_id, v_tenant,
    1, 101, 'playoff', 'pendiente'::match_status
    ...
    BEGIN
      ...
      UPDATE public.tournament_matches
         SET winner_registration_id = v_winner, score = v_score,
             status = 'jugado'::match_status, played_at = now() - interval '12 hours'
       WHERE id = v_qi;
