-- Piece 4 (hybrid incremental): fill positions.ranking in user_profile_summary so the
-- Home level card shows the user's club ranking position. Same core sources as club_ranking.
...
begin
  ...
  if _pr.user_id is not null then
    ...
    -- The user's club (first active 'club' membership) and their rank within it for this sport.
    select s.id into _club
      from public.space s
      join public.space_membership m on m.space_id = s.id
     where s.type = 'club' and m.player_id = _user_id and m.status = 'active'
     limit 1;

    if _club is not null then
      select rnk into _ranking from (
        select ss.player_id,
               row_number() over (order by pr2.nivel desc, pr2.rating desc) as rnk
          from public.space_standing ss
          join public.player_ratings pr2
            on pr2.user_id = ss.player_id and pr2.sport = _sport_key and pr2.format = _format
         where ss.space_id = _club
      ) t where t.player_id = _user_id;
    end if;
  end if;
  ...
  return jsonb_build_object(
    ...
    'positions', jsonb_build_object('ranking', _ranking, 'ladder', null, 'ladder_status', null),
