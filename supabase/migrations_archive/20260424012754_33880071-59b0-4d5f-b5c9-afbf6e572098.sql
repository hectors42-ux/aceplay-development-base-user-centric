-- RPC: historial de partidos unificado de un usuario (jugados + pendientes de carga)
-- - SECURITY DEFINER, valida tenant igual que user_profile_summary
-- - Si _user_id = auth.uid(): hasta 50 partidos jugados + todos los pendientes
-- - Si es otro usuario del mismo tenant: máx 10 partidos jugados, sin pendientes
CREATE OR REPLACE FUNCTION public.user_match_history(
  _user_id uuid,
...
BEGIN
  ...
  -- Partidos jugados (desde rating_history, mismo criterio que user_profile_summary)
  ...
  FROM (
    SELECT
      ...
      rh.source_ref_id,
      -- Datos del rival y marcador
      CASE
        WHEN rh.source = 'desafio_ladder' THEN (
        ...
        WHEN rh.source = 'partido_torneo' THEN (
          SELECT CASE
        ...
        WHEN rh.source = 'amistoso' THEN NULL
        ELSE NULL
      END AS opponent_id,
      CASE
        WHEN rh.source = 'desafio_ladder' THEN (
  ...
  -- Pendientes (solo para el dueño del perfil)
  IF v_is_self THEN
    -- Torneos: partidos con ambos contendientes asignados, sin resultado registrado, donde el usuario es jugador
    ...
    -- Pirámide: desafíos aceptados/programados sin resultado, o con propuesta del rival a confirmar
    ...
    FROM (
      SELECT
        ...
        -- needs_action_kind: 'submit' si nadie propuso aún, 'confirm' si el rival propuso y debo confirmar
        CASE
...
GRANT EXECUTE ON FUNCTION public.user_match_history(uuid, integer) TO authenticated;