...
-- Fase A: Fit Matrix v2 — breakdown con 6 señales

CREATE OR REPLACE FUNCTION public.compute_partner_fit_breakdown(_me uuid, _them uuid)
RETURNS jsonb
LANGUAGE plpgsql
...
BEGIN
  ...
  FROM public.player_ratings pr WHERE pr.user_id = _me ORDER BY pr.updated_at DESC LIMIT 1;

  SELECT pr.level, pr.matches_played, pr.last_match_at
    INTO v_their_level, v_their_matches, v_their_last
  FROM public.player_ratings pr WHERE pr.user_id = _them ORDER BY pr.updated_at DESC LIMIT 1;
  ...
  -- Frecuencia (basada en matches_played + recencia)
  ...
  ELSE
    -- balance W-L bueno (cercano a 50/50) suma; revancha reciente boost
...
GRANT EXECUTE ON FUNCTION public.compute_partner_fit_breakdown(uuid, uuid) TO authenticated;

-- Re-exportar get_partner_suggestions con breakdown
...
BEGIN
  ...
  WITH base AS (
    SELECT
      ...
      public.compute_partner_fit_breakdown(v_uid, p.user_id) AS bd
...
GRANT EXECUTE ON FUNCTION public.get_partner_suggestions(integer) TO authenticated;
