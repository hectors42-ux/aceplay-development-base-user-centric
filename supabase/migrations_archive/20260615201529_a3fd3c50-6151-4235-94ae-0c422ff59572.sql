...
-- register_to_category: nuevo parámetro _session_availability
CREATE OR REPLACE FUNCTION public.register_to_category(
  _category_id uuid,
  _player2_user_id uuid DEFAULT NULL,
  _session_availability uuid[] DEFAULT '{}'::uuid[]
)
...
BEGIN
  ...
  -- Si el torneo tiene sesiones definidas, exigir al menos 1
  SELECT COUNT(*) INTO v_session_count
  FROM public.tournament_sessions
  WHERE tournament_id = v_category.tournament_id;

  IF v_session_count > 0 AND COALESCE(array_length(_session_availability,1),0) = 0 THEN
    RAISE EXCEPTION 'Debes confirmar disponibilidad para al menos una sesión';
  END IF;

  v_is_americano := (v_category.motor::text = 'americano_rotacion');

  IF v_is_americano THEN
    IF _player2_user_id IS NOT NULL THEN
      RAISE EXCEPTION 'El americano de rotación es individual: no se acepta pareja';
    END IF;
    v_initial_status := 'pendiente_admin';
  ELSIF v_category.discipline = 'tenis_dobles' THEN
  ...
  INSERT INTO public.tournament_registrations (
    ...
    player1_user_id, player2_user_id, status, session_availability
  ) VALUES (
    v_category.tournament_id, _category_id, v_category.tenant_id,
    v_user_id, _player2_user_id, v_initial_status, COALESCE(_session_availability,'{}'::uuid[])
  ) RETURNING * INTO v_registration;

  -- Telemetría
  INSERT INTO public.tournament_events (tournament_id, tenant_id, kind, payload, actor)
  VALUES (v_category.tournament_id, v_category.tenant_id, 'player_registered',
          jsonb_build_object('category_id', _category_id, 'sessions', _session_availability),
          v_user_id);
...
-- generate_americano_round: nuevo parámetro _session_id
...
BEGIN
  ...
  SELECT array_agg(player1_user_id ORDER BY random())
   ...
   WHERE tournament_category_id = _category_id
     ...
     AND (
       _session_id IS NULL
       OR session_availability = '{}'::uuid[]
       OR _session_id = ANY(session_availability)
     );

  IF v_players IS NULL OR array_length(v_players,1) < 4 THEN
    RAISE EXCEPTION 'Se necesitan al menos 4 jugadores disponibles en esta sesión';
  END IF;
